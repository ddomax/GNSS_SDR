...
ifData      = ifData + 0.01*rand(1,baseLen);
%% Write data file
fid = fopen('./myGNSSdata.bin','wb');
fwrite(fid,int8(ifData*2),settings.dataType);
fclose(fid);
% fid = fopen('./myGNSS_NO_PN.bin','wb');
% fwrite(fid,int8(ifData*2),settings.dataType);
% fclose(fid);
end